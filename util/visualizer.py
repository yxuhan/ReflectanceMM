...
class MyVisualizer:
    ...
    def display_current_results(self, visuals, total_iters, epoch, dataset='train', save_results=False, count=0, name=None,
        ...
        for label, image in visuals.items():
            for i in range(image.shape[0]):
                ...
                if save_results:
                    save_path = os.path.join(self.img_dir, dataset, '%s_epoch_%s_%06d' % (self.opt.name, epoch, total_iters))
    ...
    def save_mm_visual_results(self, model_diff_uv, model_spec_uv, point_light_render_list, total_iters, epoch, dataset='train'):
        cur_prefix = os.path.join(self.img_dir, dataset, '%s_epoch_%s_%06d' % (self.opt.name, epoch, total_iters))
        
        diff_model_path = os.path.join(cur_prefix, 'diff_model')
        if not os.path.isdir(diff_model_path):
            os.makedirs(diff_model_path)
        for i in range(model_diff_uv.shape[0]):
            tv_save_image(model_diff_uv[i], os.path.join(diff_model_path, "%03d.png" % i))

        spec_model_path = os.path.join(cur_prefix, 'spec_model')
        if not os.path.isdir(spec_model_path):
            os.makedirs(spec_model_path)
        for i in range(model_spec_uv.shape[0]):
            cur_spec = model_spec_uv[i]
            n, c, h, w = cur_spec.size()
            vis_spec = cur_spec.transpose(0, 1).reshape(n * c, 1, h, w)
            tv_save_image(vis_spec, os.path.join(spec_model_path, "%03d.png" % i), nrow=n)
        
        for i in range(point_light_render_list.shape[0]):
            save_path = os.path.join(cur_prefix, 'output_vis_%03d' % (i))
            if not os.path.isdir(save_path):
                os.makedirs(save_path)
            cur_render = point_light_render_list[i]
            for j in range(cur_render.shape[0]):
                cur_vis = cur_render[j]
                tv_save_image(cur_vis, os.path.join(save_path, "%03d.png" % j))
    ...
    def save_coeff_stat(self, diff_coeff_buf, epoch, total_iters, dataset):
        cur_prefix = os.path.join(self.img_dir, dataset, '%s_epoch_%s_%06d' % (self.opt.name, epoch, total_iters))
        x, y = cnt_bin(diff_coeff_buf.detach().cpu()) 
        plt.cla()
        plt.bar(y, x)
        plt.savefig(os.path.join(cur_prefix, "diff_stat.png"))
        torch.save(diff_coeff_buf.detach().cpu(), os.path.join(cur_prefix, "diff_stat.pkl"))


def cnt_bin(a, binsize=11):
    ma = 4
    mi = -4
    step = (ma - mi) / binsize
    x = []
    ...
    for i in range(binsize):
        high = (i + 1) * step + mi
        low = i * step + mi
        x.append(torch.logical_and(a <= high, a >= low).float().sum().item())
        y.append("%.2f" % ((high + low) / 2))
    return x, y
