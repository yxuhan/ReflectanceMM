...
class BaseOptions():
    ...
    def initialize(self, parser):
        ...
        parser.add_argument('--renderer', type=str, default='pytorch3d', help='name of the experiment. It decides where to store samples and models')
        parser.add_argument('--gpu_ids', type=str, default='', help='gpu ids: e.g. 0  0,1,2, 0,2. use -1 for CPU')
        parser.add_argument('--gpu_nums', type=int, default=1, help='number of used gpus')
        ...
        parser.add_argument('--add_image', type=util.str2bool, nargs='?', const=True, default=False, help='whether add image to tensorboard')
        parser.add_argument('--world_size', type=int, default=1, help='batch nums of images for evaluation')

        # model parameters
        parser.add_argument('--model', type=str, default='refmm', help='chooses which model to use. [baseline | pbr_sh | pbr_lm]')
    ...
    def parse(self):
        ...
        else:  # for sensetime server
            opt.gpu_ids = os.environ['CUDA_VISIBLE_DEVICES']
        print("Using cuda: %s" % os.environ['CUDA_VISIBLE_DEVICES'])
